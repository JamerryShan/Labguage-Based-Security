  ...
  .get('/download/users', adminRequired, (req, res) => {
    mongo.db.collection('users')
      .find().toArray(function(err, result) {    
        ...
        res.end(serializer.stringify(result, { isSerializable: function(item, key) {
          return item.hasOwnProperty(key) && key != 'img' && key != '_id';
      }}));
    })
  })
    ...
    newUsers.forEach(item => {
      ...
      mongo.db.collection('users')
        .findOne({ lcUsername: item.lcUsername }, {collation: {locale: "en", strength: 2}}, (err, user) => {
          ...
          if (user) {
            console.log('The user', item.lcUsername, 'already exists')
          }
          else {
